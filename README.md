...
- 🔥 **צריבת כתוביות על וידאו (Burn-In)** — גרור **וידאו + SRT יחד** ומקבל וידאו חדש (`movie.subtitled.mp4`) עם הכתוביות צרובות בתוך התמונה, ב-**9 סגנונות** לבחירה (קלאסי, קופסה, **הבלטת מילים בסגנון קריוקי/CapCut**, מודגש, זוהר, צל רך, עדין, קופצני, עולה) בטאב הגדרות ייעודי. רינדור עברית RTL מדויק (libass), הצריבה מקומית לחלוטין והמקור לא משתנה. הבלטת מילים זמינה לכתוביות שנוצרו בתמלול הווידאו של Timluli (קובץ `words.json` עם תזמוני מילים נשמר אוטומטית לצד ה-SRT); בלעדיו הצריבה יורדת בעדינות לסגנון קופסה
...
- 🚻 התאמת מגדר בתרגום כתוביות (ניסיוני, כבוי כברירת מחדל) — זיהוי קולי מקומי של מגדר הדוברים (לפי גובה הקול) משפר נטיות זכר/נקבה בתרגום לעברית; שום אודיו לא נשלח
...
- **התאמת מגדר בתרגום (ניסיוני):** הניתוח האקוסטי של קולות הדוברים (לפי גובה הקול) רץ **מקומית לחלוטין** בעת הפקת הכתוביות — שום אודיו לא נשלח. לספק התרגום מצורפות לכל היותר תגיות טקסט `[M]`/`[F]` כחלק מהכתוביות.

### צריבת כתוביות ותזמוני מילים

- **הצריבה מקומית לחלוטין** — ffmpeg רץ על המחשב; שום דבר לא נשלח לשרת.
- תמלול וידאו→SRT שומר לצד הכתוביות קובץ `words.json` (תזמוני מילים לסגנון "הבלטת מילים"). **הקובץ מכיל את טקסט התמלול**, נשמר מקומית בלבד לצד ה-SRT ולעולם לא נשלח לשום מקום. אפשר למחוק אותו בכל עת — הצריבה תרד אוטומטית לסגנון קופסה.
...
| צריבת כתוביות (מקומי) | לא | קובץ פלט מקומי | לא |
...
- ✅ **צריבת כתוביות (Burn-In) + סגנונות** — גרירת וידאו+SRT יחד צורבת את הכתוביות לתוך הווידאו (ffmpeg/libass, מקומי לחלוטין), עם 9 סגנונות מובנים (כולל זוהר ניאון, מודגש-רשתות, צל רך ועולה) וטאב הגדרות ייעודי בעיצוב צ'יפים קומפקטי
- ✅ **הבלטת מילים (קריוקי) בצריבה** — תזמוני מילים מ-Groq/whisper מקומי נשמרים כ-`words.json` לצד ה-SRT; בצריבה המילה הנאמרת נצבעת בזמן אמת (תגי `\k` של ASS, אומת ויזואלית RTL). עריכת SRT ידנית לא שוברת כלום — שורה שלא תואמת מרונדרת רגיל
- ✅ **חלון הגדרות מעוצב מחדש** — ממשק כרטיסים עם אייקונים ולוגו, מסך בית/מדריך, שמירה אוטומטית ומצב בהיר/כהה
- ✅ **התאמת מגדר בתרגום כתוביות (ניסיוני)** — זיהוי אקוסטי מקומי של מגדר הדובר לכל כתובית (לפי תדר היסוד של הקול, ללא הורדות וללא ענן); התרגום לעברית מקבל תגיות `[M]`/`[F]` ובוחר נטיות זכר/נקבה נכונות. שמרני בכוונה — בספק לא מתויג. מגבלות V1: קולות ילדים עשויים להיות מסווגים כנקבה, ושני דוברים באותה כתובית נופלים ל"לא ידוע". V2 מתוכנן: diarization מלא (sherpa-onnx) ברמת הדובר
